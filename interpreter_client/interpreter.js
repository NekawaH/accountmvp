...
async function initCloudProject() {
    try {
        const response = await fetch(`${API_URL}/projects/${GUEST_ID}`);
        const data = await response.json();
        
        window.vfs = data.vfs;
        window.currentFile = Object.keys(window.vfs)[0];
        ...
        // Fallback to your existing default if server is down
        window.vfs = { "main.psc": codeEl.value };
        window.currentFile = "main.psc";
    }
}
...
async function saveToCloud() {
    // Sync the current editor content to the VFS first
    window.vfs[window.currentFile] = codeEl.value;

    try {
        await fetch(`${API_URL}/projects/${GUEST_ID}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ vfs: window.vfs })
        });
