          ...
          <div className="space-y-6">
            {received.length > 0 && (
              <div className="space-y-3">
                ...
                {received.map(inv => (
                  <div key={inv.id} className={`bg-white border rounded-xl px-5 py-4 shadow-sm flex items-center gap-4 ${(inv.status === 'REMOVED' || inv.status === 'LEFT') && !inv.seenByTo ? 'border-blue-300' : 'border-gray-200'}`}>
                    {inv.from.avatarUrl
                    ...
                    <div className="flex-1 min-w-0 flex items-center gap-2">
                      <div className="min-w-0">
                        <p className="text-sm text-gray-800">
                          ...
                          {inv.status === 'REMOVED'
                            ...
                            : inv.status === 'LEFT'
                              ? <>{' '}left your workspace <span className="font-semibold">{inv.workspace.name}</span></>
                      ...
                      {(inv.status === 'REMOVED' || inv.status === 'LEFT') && !inv.seenByTo && (
                    ...
                    {inv.status === 'PENDING' ? (
                      ...
                      <span className={`text-xs font-medium px-2.5 py-1 rounded-full flex-shrink-0 ${
                        ...
                        inv.status === 'LEFT' ? 'bg-orange-100 text-orange-600' :
                        'bg-gray-100 text-gray-500'
                      }`}>
                        {inv.status === 'ACCEPTED' ? 'Accepted' : inv.status === 'REMOVED' ? 'Removed' : inv.status === 'LEFT' ? 'Left' : 'Declined'}
                      </span>
                    )}
                  </div>
                ))}
